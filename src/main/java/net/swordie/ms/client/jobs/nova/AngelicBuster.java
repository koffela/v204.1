...
public class AngelicBuster extends Job {
        ...
        if (tsm.hasStat(AngelicBursterSoulSeeker)) {
            ...
            for (MobAttackInfo mai : attackInfo.mobAttackInfo) {
                ...
                if (Util.succeedProp(TW1prop)) {
                    ...
                    ForceAtomInfo forceAtomInfo = new ForceAtomInfo(1, inc, 20, 40, anglenum, 0, (int) System.currentTimeMillis(), 1, 0, new Position(5, 0)); // Slightly behind the player
                    chr.getField().broadcastPacket(FieldPacket.createForceAtom(false, 0, chr.getId(), type, true, mobID, SOUL_SEEKER_ATOM, forceAtomInfo, new Rect(), 0, 300, mob.getPosition(), SOUL_SEEKER_ATOM, mob.getPosition()));
                }
            }
        }
    }
    ...
    private void createSoulSeekerForceAtom() {
        ...
        int inc = ForceAtomEnum.AB_ORB.getInc();
        int type = ForceAtomEnum.AB_ORB.getForceAtomType();
        ForceAtomInfo forceAtomInfo = new ForceAtomInfo(1, inc, 20, 40, anglenum, 250, (int) System.currentTimeMillis(), 1, 0, new Position(0, -100));
        chr.getField().broadcastPacket(FieldPacket.createForceAtom(false, 0, chr.getId(), type, true, mobID, SOUL_SEEKER_ATOM, forceAtomInfo, new Rect(), 0, 300, life.getPosition(), SOUL_SEEKER_ATOM, life.getPosition()));
    }
    ...
    private void recreateSoulSeekerForceAtom(AttackInfo attackInfo) {
        ...
        for (MobAttackInfo mai : attackInfo.mobAttackInfo) {
            ...
            if (Util.succeedProp(TW1prop)) {
                ...
                ForceAtomInfo forceAtomInfo = new ForceAtomInfo(1, inc, firstimpact, 2, anglenum, 0, (int) System.currentTimeMillis(), 1, 0, new Position());
                chr.getField().broadcastPacket(FieldPacket.createForceAtom(true, chr.getId(), mobID, type, true, mobID, SOUL_SEEKER_ATOM, forceAtomInfo, new Rect(), 0, 300, mob.getPosition(), SOUL_SEEKER_ATOM, mob.getPosition()));
            }
        }
    }
