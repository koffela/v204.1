...
public class MobTemporaryStat {
     ...
     * 
     * @param skillID
     *            the skill id of which the burn should exist
     * @param ownerCID
     *            the burn skill's owner's id that should match (or 0 if ignored)
    ...
    public void createAndAddBurnedInfo(Char chr, Skill skill) {
        ...
        ScheduledFuture burn = EventManager.addFixedRateEvent(() -> getMob().damage(chr, damage), bi.getAttackDelay() + bi.getInterval(), bi.getInterval(), bi.getDotCount());
