...
public class Char {
    ...
    public static Char getFromDBByNameAndWorld(String name, int worldId) {
        Session session = DatabaseManager.getSession();
        Transaction transaction = session.beginTransaction();
        Query query = session.createQuery("FROM Char chr " + "WHERE chr.avatarData.characterStat.name = :name AND chr.avatarData.characterStat.worldIdForLog = :world");
        query.setParameter("name", name);
    ...
    public void addItemToInventory(InvType type, Item item, boolean hasCorrectBagIndex) {
        ...
        if (inventory != null) {
            ...
            if (existingItem != null && existingItem.getInvType().isStackable() && existingItem.getQuantity() < ii.getSlotMax()) {
                ...
                write(WvsContext.inventoryOperation(true, false, UpdateQuantity, (short) existingItem.getBagIndex(), (byte) -1, 0, existingItem));
    ...
    public List<Inventory> getInventories() {
        return new ArrayList<>(Arrays.asList(getEquippedInventory(), getEquipInventory(), getConsumeInventory(), getEtcInventory(), getInstallInventory(), getCashInventory()));
    }
     ...
     * @param skill
     *            The Skill this Char should get.
     * @param addRegardlessOfLevel
     *            if this is true, the skill will not be removed from the char, even if the cur level
     ...
     * 
     * @param skillID
     *            the id of the skill that should be removed
     */
    ...
    public void unequip(Item item) {
        ...
        int replacement = item.isCash() ? getEquippedInventory().getItems().stream().mapToInt(Item::getItemId).filter(i -> ItemConstants.getItemPrefix(i) == ItemConstants.getItemPrefix(itemID)).findFirst().orElse(-1) : -1;
     ...
     *            the ID of the field to warp to
     * @param portalId
     *            the ID of the portal where the Char should spawn
     */
    ...
    public void setNearestReturnPortal() {
        Rect rect = new Rect(new Position(getPosition().getX() - 30, getPosition().getY() - 30), new Position(getPosition().getX() + 50, // wide girth
                getPosition().getY() + 50));
    ...
    public void warp(Field toField, Portal portal, boolean characterData, boolean saveReturnMap) {
        ...
        getClient().write(Stage.setField(this, toField, getClient().getChannel(), false, 0, characterData, hasBuffProtector(), (byte) portal.getId(), false, 100, null, true, -1));
        if (characterData) {
            initSoulMP();
            Party party = getParty();
            if (party != null) {
            ...
            for (Friend f : getFriends()) {
                f.setFlag(getClient().getWorld().getCharByID(f.getFriendID()) != null ? FriendFlag.FriendOnline : FriendFlag.FriendOffline);
            }
            for (Friend f : getAccount().getFriends()) {
                f.setFlag(getClient().getWorld().getAccountByID(f.getFriendAccountID()) != null ? FriendFlag.AccountFriendOnline : FriendFlag.AccountFriendOffline);
            }
        }
     ...
     *            The amount of exp to add
     * @param eii
     *            The info to send to the client
     */
    ...
    public void addTraitExp(Stat traitStat, int amount) {
        ...
        if (amount <= 0 || traitExp >= GameConstants.MAX_TRAIT_EXP) {
            return;
        } else if (traitExp + amount > GameConstants.MAX_TRAIT_EXP) {
    ...
    public void setDamageSkin(int itemID) {
        setDamageSkin(new DamageSkinSaveData(ItemConstants.getDamageSkinIDByItemID(itemID), itemID, false, StringData.getItemStringById(itemID)));
    }
    ...
    public void setPremiumDamageSkin(int itemID) {
        setPremiumDamageSkin(new DamageSkinSaveData(ItemConstants.getDamageSkinIDByItemID(itemID), itemID, false, StringData.getItemStringById(itemID)));
    }
    ...
    public boolean addDrop(Drop drop) {
        if (drop.isMoney()) {
            ...
            if (itemID == GameConstants.BLUE_EXP_ORB_ID || itemID == GameConstants.PURPLE_EXP_ORB_ID || itemID == GameConstants.RED_EXP_ORB_ID) {
    ...
    public boolean hasItemCount(int itemID, int count) {
        ...
        return inv.getItems().stream().filter(i -> i.getItemId() == itemID).mapToInt(Item::getQuantity).sum() >= count;
    }
    ...
    public void setOnline(boolean online) {
        if (getGuild() != null) {
            ...
            if (ally != null) {
                ally.broadcast(WvsContext.allianceResult(AllianceResult.notifyLoginOrLogout(ally, g, gm, !this.online && online)), this);
            } else {
                getGuild().broadcast(WvsContext.guildResult(GuildResult.notifyLoginOrLogout(g, gm, online, online)), this);
            }
        }
        this.online = online;
        if (getParty() != null) {
    ...
    public void removePersonalNpcByObjectId(int objectID, boolean packet) {
        ...
        if (npc != null) {
            if (packet)
                write(NpcPool.npcLeaveField(npc));
     ...
     *
     * @param items
     *            the list of items this char should be able to hold
    ...
    public boolean addDamageSkin(int itemID) {
        ...
        if (error != null) {
            ...
            setDamageSkin(dssd);
            write(UserLocal.damageSkinSaveResult(DamageSkinType.Req_Reg, DamageSkinType.Res_Success, client.getChr()));
    ...
    public Pet getPetByIdx(int idx) {
        return getPets().stream().filter(p -> p.getIdx() == idx).findAny().orElse(null);
    }
     ...
     *
     * @param skillID
     *            the skill id of the skill to put on cooldown
     ...
     *
     * @param exp
     *            the exp to add (may be negative)
     */
    ...
    public Set<LinkSkill> getLinkSkills() {
        return getAccount().getLinkSkills().stream().filter(ls -> ls.getOwnerID() != getId()).collect(Collectors.toSet());
    }
     ...
     *
     * @param skillID
     *            the skill's id to add
     * @param currentLevel
     *            the current level of the skill
     * @param masterLevel
     *            the master level of the skill
     */
     ...
     *            The BaseStat
     * @param amount
     *            the amount of BaseStat to remove
     */
    ...
    public void addItemToInventory(int id, int quantity) {
        if (ItemConstants.isEquip(id)) {  // Equip
            ...
            getClient().write(WvsContext.inventoryOperation(true, false, Add, (short) equip.getBagIndex(), (byte) -1, 0, equip));

        } else {    // Item
            Item item = ItemData.getItemDeepCopy(id);
            item.setQuantity(quantity);
            addItemToInventory(item);
            getClient().write(WvsContext.inventoryOperation(true, false, Add, (short) item.getBagIndex(), (byte) -1, 0, item));
    ...
    public String toString() {
        return "Char{" + "(" + super.toString() + ")id=" + id + ", accId=" + accId + ", name=" + getName() + '}';
    }
    ...
    public void initBlessingSkillNames() {
        ...
        for (Char chr : account.getCharacters()) {
            if (!chr.equals(this) && chr.getLevel() >= 10 && (fairyChar == null || chr.getLevel() > fairyChar.getLevel())) {
                fairyChar = chr;
            }
        }
        if (fairyChar != null) {
        ...
        for (Char chr : account.getCharacters()) {
            if (!chr.equals(this) && (JobConstants.isCygnusKnight(chr.getJob()) || JobConstants.isMihile(chr.getJob()) && chr.getLevel() >= 5 && (empressChar == null || chr.getLevel() > empressChar.getLevel()))) {
                empressChar = chr;
            }
        }
        if (empressChar != null) {
    ...
    public void setMatrixInventory(MatrixInventory matrixInventory) {
        this.matrixInventory = matrixInventory;
    }

    public void setVMatrixSlots(int slots) {
        this.vmatrixslots = slots;
    }

    public int getBonusVMatrixSlots() {
        return vmatrixslots;
    }
     ...
     * 
     * @param override
     *            Whether or not to override the old Android if one exists.
     */
