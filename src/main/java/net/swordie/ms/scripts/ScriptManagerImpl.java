...
public class ScriptManagerImpl implements ScriptManager {
    ...
    private void startScript(String name, ScriptType scriptType) {
        ...
        if (!exists) {
            ...
            if (ServerConfig.AUTO_CREATE_UNCODED_SCRIPTS) {
                ...
                try {
                    ...
                    switch (scriptType) {
                        ...
                        case Reactor:
                        case Npc:
                            content.addAll(Util.makeSet("# Object Position X: " + getObjectPositionX(), "# Object Position Y: " + getObjectPositionY()));
                    }
            ...
            dir = String.format("%s/%s/%s%s", ServerConstants.SCRIPT_DIR, scriptType.getDir().toLowerCase(), DEFAULT_SCRIPT, SCRIPT_ENGINE_EXTENSION);
        }
        ScriptInfo si = getScriptInfoByType(scriptType);
        si.setActive(true);
        CompiledScript cs;
        ...
        try {
            ...
            if (si.isActive() && name.equals(si.getScriptName()) && ((scriptType != ScriptType.Field && scriptType != ScriptType.FirstEnterField) || (chr != null && chr.getFieldID() == si.getParentID()))) {
     ...
     *            mob id to kill
     * @param killAll
     *            whether or not to kill all of them or just the first one
     */
    ...
    public Drop getDropInRect(int itemID, Rect rect) {
        ...
        return field.getDropsInRect(rect).stream().filter(drop -> drop.getItem() != null && drop.getItem().getItemId() == itemID).findAny().orElse(null);
    }

    @Override
    public Drop getDropInRect(int itemID, int rectRange) {
        return getDropInRect(itemID, new Rect(new Position(chr.getPosition().getX() - rectRange, chr.getPosition().getY() - rectRange), new Position(chr.getPosition().getX() + rectRange, chr.getPosition().getY() + rectRange)));
    ...
    public void showHP(int templateID) {
        chr.getField().getMobs().stream().filter(m -> m.getTemplateId() == templateID).findFirst().ifPresent(mob -> chr.getField().broadcastPacket(FieldPacket.fieldEffect(FieldEffect.mobHPTagFieldEffect(mob))));
    }

    @Override
    public void showHP() {
        chr.getField().getMobs().stream().filter(m -> m.getHp() > 0).findFirst().ifPresent(mob -> chr.getField().broadcastPacket(FieldPacket.fieldEffect(FieldEffect.mobHPTagFieldEffect(mob))));
    }
    ...
    public void changeReactorState(int reactorId, byte state, short delay, byte stateLength) {
        Field field = chr.getField();
        Reactor reactor = field.getReactors().stream().filter(r -> r.getObjectId() == getObjectIDByScriptType(ScriptType.Reactor)).findAny().orElse(null);
        if (reactor == null) {
            return;
        }
        reactor.setState(state);
        ...
        for (int i = 0; i < amount; i++) {
            if (Util.succeedProp(proc)) {
                ...
                if (foothold != null) {
                    ...
                    obtacleAtomInfosSet.add(new ObtacleAtomInfo(oae.getType(), key, position, new Position(), oae.getHitBox(), damage, 0, 0, height, 0, velocity, height, angle));
                }
            }
        }
    ...
    public void showBalloonMsgOnNpc(String path, int duration, int x, int y, int templateID) {
        ...
        if (objectID == 0)
            return;
        chr.write(UserLocal.inGameDirectionEvent(InGameDirectionEvent.effectPlay(path, duration, new Position(x, y), 0, objectID, false, 0)));
    }
    ...
    public void showNpcEffectOnPosition(String path, int x, int y, int templateID) {
        ...
        if (objectID == 0)
            return;
        chr.write(UserLocal.inGameDirectionEvent(InGameDirectionEvent.effectPlay(path, 0, new Position(x, y), 0, objectID, false, 0)));
    }
    ...
    public void blind(int enable, int x, int color, int unk1, int unk2, int time) {
        chr.write(FieldPacket.fieldEffect(FieldEffect.blind(enable, x, color, unk1, unk2, time, 0)));
    }

    public void blind(int enable, int x, int color, int unk1, int unk2, int time, int unk3) {
        chr.write(FieldPacket.fieldEffect(FieldEffect.blind(enable, x, color, unk1, unk2, time, unk3)));
    }

    public void OnOffLayer_On(int term, String key, int unk1, int unk2, int z, String path, int origin, int unk5, int unk6, int unk7) {
        chr.write(FieldPacket.fieldEffect(FieldEffect.OnOffLayer_On(term, key, unk1, unk2, z, path, origin, unk5, unk6, unk7)));
    }
    ...
    public ScheduledFuture invokeAtFixedRate(long initialDelay, long delayBetweenExecutions, int executes, String methodName, Object... args) {
        ScheduledFuture scheduledFuture;
        if (executes == 0) {
            scheduledFuture = EventManager.addFixedRateEvent(() -> invoke(this, methodName, args), initialDelay, delayBetweenExecutions);
        } else {
            scheduledFuture = EventManager.addFixedRateEvent(() -> invoke(this, methodName, args), initialDelay, delayBetweenExecutions, executes);
        }
    ...
    public void returnPinkZakum() {
        ...
        int warpMap = e instanceof PinkZakumEvent ? PinkZakumEvent.BATTLE_MAP : chr.getPreviousFieldID();
